...
## 2026-09-23 — Sandboxed code-execution grading: 36/36, and a real extractor bug caught first

Built `backend/app/evaluation/sandbox.py`: a sandboxed grader for the 13
coding tasks and the 5 revised debugging tasks (009-013), using real
kernel-enforced isolation (a fresh network namespace with no interfaces,
a fresh mount namespace with the root filesystem read-only and only a
per-run temp dir writable) — verified directly (a blocked socket connect,
a blocked filesystem write), not assumed. Graded the real Groq responses
for these 18 tasks x 2 models.

**First run: 33/36, with 3 failures on `groq-gpt-oss-20b`.** Read the
actual response text behind one before trusting that number and found
the real cause: debugging-style responses commonly quote the original
buggy function before presenting the fix, so a response can contain 2-4
fenced code blocks that all define the same function name.
`extract_python_code` took the *first* match — the unfixed original —
not the model's actual answer. All 3 "failures" were the grader grading
the wrong code, confirmed by inspecting each one directly. Fixed (last
matching block, not first), added a regression test with the exact
buggy-then-fixed pattern from the real data, re-ran: **36/36.**

`experiments/results/groq-gpt-oss-expansion.json`'s `evaluation_status`
for these 36 (task, model) pairs was then updated from `not_evaluated`
to `correct` (`scripts/apply_code_grading_results.py`) — an evaluation-
quality fix, not a training-data change: each task's `evaluation_type`
in `benchmarks/tasks/*.json` stays `manual`, so
`backend/app/routing/learned/dataset.py`'s row-generation gate is
untouched (`build_training_rows()` still returns exactly 56 rows,
confirmed unchanged). These 18 tasks still contribute zero V3 training
rows until wiring `evaluation_type` is made as its own deliberate
decision.

## 2026-09-23 — Routing opportunity analysis: does this model pair even offer one?

Before any further training, asked a prior question: for `groq-gpt-
oss-20b` vs `groq-gpt-oss-120b`, on the 104-task benchmark, is there a
real quality gap between them for a router to exploit at all? Full data
and every table: `experiments/results/routing-opportunity-analysis.md`
(generated by `scripts/analyze_routing_opportunity.py`, reproducible
from the committed results file).

**75 of 104 tasks have real ground truth for both models** (57 auto-
graded + 18 now code-graded); 29 remain ungraded (8 debugging, 8
reasoning, 13 summarization — all still `evaluation_type=manual`, no
grader exists for any of them yet).

**On the 75 graded tasks:**
- both models correct: 65 (86.7%)
- only 20b correct: 3 (`extraction-002`, `extraction-005`, `math-011`)
- only 120b correct: 3 (`classification-012`, `extraction-008`, `extraction-012`)
- both incorrect: 4 (`classification-010`, `extraction-004`, `math-005`, `math-013`)

**The decisive fact: always-20b and always-120b score identically —
68/75 (90.7%) each** — the "only X correct" counts are exactly tied (3
vs 3), so neither model is a better unconditional default than the
other on this task set. A perfect oracle router (always picks whichever
one is right, on the 6 tasks where they disagree) reaches 71/75
(94.7%) — a ceiling only 4.0 percentage points above either model
alone, defined by just 6 tasks total out of 75 graded ones.

Per-category, coding/debugging/reasoning/structured_output show **zero**
quality difference (both models 100% — though reasoning's graded subset
is only the 5 easiest, deterministic tasks, and debugging's is the 5
revised, well-specified ones; the harder, still-ungraded tasks in both
categories are excluded from this by construction, not because they're
solved). Math is the one category where 120b is *worse* (76.9% vs
84.6%, though N=13 makes a 1-task swing a 7.7-point change — not
strong evidence either way). 120b costs 1.87x the nominal tokens and
takes 1.51x the latency of 20b, for that ~4-point theoretical ceiling.

**Answer to "do we have a real routing opportunity": not really, not
with this model pair on this task distribution.** The opportunity that
exists is small (4 points), defined by very few tasks (6), and not
obviously learnable from that few examples — consistent with, not
contradicted by, V3's own repeated finding that a trivial single-model
baseline is hard to beat. This doesn't mean routing is a dead end for
Switchyard generally; it means this specific pair is too similar in
capability, and/or the benchmark's graded portion is not hard enough to
separate them, for intelligent routing to have much room to help. A
model pair with a starker capability gap (e.g. a much smaller/cheaper
